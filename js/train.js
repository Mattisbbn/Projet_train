...
function dessine_case(contexte, plateau, x, y) {
    ...
    // if(la_case === "IMAGE_RAIL_HORIZONTAL"){
...
function tchou() {
    ...
    simulateur.addEventListener("click", function (event) {
        ...
        if (
            mouseX >= 0 &&
            mouseX < plateau.largeur &&
            mouseY >= 0 &&
            mouseY < plateau.hauteur
        ) {
                ...
                if (button.disabled === true) {
                    ...
                    switch (button.id) {
                        ...
                        case "bouton_rail_horizontal":
                            plateau.cases[mouseX][mouseY] = Type_de_case.gray;
                        ...
                        case "bouton_rail_droite_vers_haut":
                            plateau.cases[mouseX][mouseY] = Type_de_case.gray;
                            dessine_case(contexte, plateau, mouseX, mouseY);
                            plateau.cases[mouseX][mouseY] = Type_de_case.Rail_droite_vers_haut;
                            break;

                        case "bouton_rail_haut_vers_droite":
                            plateau.cases[mouseX][mouseY] = Type_de_case.gray;
                            dessine_case(contexte, plateau, mouseX, mouseY);
                            plateau.cases[mouseX][mouseY] = Type_de_case.Rail_haut_vers_droite;
                            break;
            ...
            function defineTrainLenght(trainlenght) {
                for (let i = 0; i < trainlenght; i++) {
                    if (i === 0) {
                        plateau.cases[mouseX][mouseY] = Type_de_case.Loco;
                        dessine_case(contexte, plateau, mouseX, mouseY);
                    }
                    if (i > 0) {
                        plateau.cases[mouseX + i][mouseY] = Type_de_case.Wagon;
                        dessine_case(contexte, plateau, mouseX + i, mouseY);
                    }
                }
            }
